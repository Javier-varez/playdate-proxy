...
fn serial_callback(message: [*:0]const u8) callconv(.C) void {
    ...
    while (iter.next()) |v| {
        ...
        args.append(v) catch {
            app_state.panic("Err: Out of memory while collecting args for command", .{});
            return;
        };
    }

    if (args.items.len == 0) {
        app_state.print("Err: Please, specify a command to handle", .{});
        return;
    }
    ...
    app_state.print("Err: Unknown command {s}", .{cmd});
}
